...
# ============================================================================
# WARMUP EVAL CALLBACK
# ============================================================================
from stable_baselines3.common.callbacks import EvalCallback as _EvalCallback


class WarmupEvalCallback(_EvalCallback):
    """EvalCallback ktory NIE zapisuje best modelu przed startem uczenia.

    Problem: przy TD3/SAC/TQC z learning_starts > 0 pierwsze ewaluacje odbywaja
    sie na NIEWYTRENOWANEJ sieci (czysty szum eksploracji + losowa inicjalizacja).
    Dla agenta makro losowe logity -> softmax -> wagi ~rowne -> przypadkowo
    silny equal-weight portfel, ktory potrafi dac najwyzsza nagrode ZANIM siec
    cokolwiek sie nauczy. Standardowy EvalCallback zapisalby ten losowy model
    jako best_model.zip.

    Rozwiazanie: dopoki num_timesteps < min_timesteps_before_best:
      - ewaluacja dalej dziala (widoczne liczby w logu),
      - ale best model NIE jest zapisywany,
      - best_mean_reward trzymany na -inf, wiec pierwsza ewaluacja PO progu
        zapisze sie jako genuinnie najlepsza NAUCZONA polityka.
    """

    def __init__(self, *args, min_timesteps_before_best: int = 0, **kwargs):
        super().__init__(*args, **kwargs)
        self.min_timesteps_before_best = int(min_timesteps_before_best)

    def _on_step(self) -> bool:
        before_threshold = self.num_timesteps < self.min_timesteps_before_best
        if before_threshold:
            # Wylacz zapis best modelu na czas warmupu
            saved_path = self.best_model_save_path
            self.best_model_save_path = None
            result = super()._on_step()
            self.best_model_save_path = saved_path
            # Trzymaj best na -inf zeby pierwsza ewaluacja PO progu wygrala
