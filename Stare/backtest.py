# ===== file: backtest.py =====
"""
Finalny backtest hierarchii macro -> fund -> tech.

Benchmarki (BEZ data leakage, metodologia NIEZMIENIANA):
  equal_weight, markowitz_min_variance, markowitz_max_sharpe.
  Markowitz fit: split=val -> train; split=test -> train+val.

Output: {outputs_dir}/backtest/{algo}/{split}/

Obsluga RecurrentPPO przez predict_action() (LSTM state).

Uruchomienie:
  python backtest.py --algo td3 --run-name td3_default --split test --prefer-model best_reward
"""
...
from utils import (set_global_seed, resolve_device, setup_logger,
                   configure_run_paths, apply_date_overrides, predict_action)
...
from metrics import (
    summarize_returns, summarize_vs_benchmark, equal_weight_benchmark,
    markowitz_min_variance_weights, markowitz_max_sharpe_weights,
...
def _resolve_agent_path(base: Path, algo: str, name: str, prefer: str) -> Path:
    ...
    order = [best, final] if prefer == "best_reward" else [final, best]
    for p in order:
        if p.exists():
            return p
...
def main():
    ...
    logger.info(f"=== BACKTEST  algo={cfg.train.algo}  run={args.run_name} "
    ...
    logger.info(f"split T={tensors.technical.shape[0]}  dates {tensors.dates[0].date()} -> {tensors.dates[-1].date()}")

    sector_ids = np.array([raw["sector_map"][t] for t in raw["tickers"]], dtype=np.int64)
    n_sectors = len(SECTOR_COLUMNS)

    def sector_rets(stock_rets):
    ...
    sec_rets = sector_rets(tensors.returns)

    base = Path(cfg.paths.outputs_dir) / "models"
    macro_path = _resolve_agent_path(base / "macro" / cfg.train.algo, cfg.train.algo, "macro", args.prefer_model)
    fund_path = _resolve_agent_path(base / "fund" / cfg.train.algo, cfg.train.algo, "fund", args.prefer_model)
    tech_path = _resolve_agent_path(base / "tech" / cfg.train.algo, cfg.train.algo, "tech", args.prefer_model)
    ...
    # Rollout — TechEnv woła macro/fund wewnetrznie; tu sterujemy TYLKO tech_agent.
    # Uwaga: macro/fund predict wewnatrz env uzywaja .predict() deterministycznie;
    # dla RecurrentPPO env nie utrzymuje ich LSTM state (rebalans co 21 dni — stan
    # resetuje sie naturalnie). LSTM state utrzymujemy dla tech_agent (codzienny).
    env = TechEnv(tensors, tensors.returns, sec_rets, sector_ids,
                   macro_agent, fund_agent, cfg)
    obs, _ = env.reset()
    done = False
    step = 0
    lstm_states = None
    ...
    while not done:
        action, lstm_states = predict_action(
            tech_agent, obs, cfg.train.algo, deterministic=True,
    ...
    sector_alloc = np.zeros((len(weights), n_sectors))
    for s in range(n_sectors):
        mask = sector_ids == s
    ...
    # Markowitz bez leakage
    if args.split == "val":
        mkt_train_returns = dataset.train.stock_returns
        logger.info(f"[markowitz] fit na train: T={mkt_train_returns.shape[0]}")
    else:
        mkt_train_returns = np.vstack([dataset.train.stock_returns, dataset.val.stock_returns])
        logger.info(f"[markowitz] fit na train+val: T={mkt_train_returns.shape[0]}")

    w_minvar = markowitz_min_variance_weights(mkt_train_returns, ridge=1e-4)
    w_maxsh = markowitz_max_sharpe_weights(mkt_train_returns, ridge=1e-4)

    bench_ew = equal_weight_benchmark(tensors.returns[:len(net)])
    bench_minvar = portfolio_returns_from_weights(tensors.returns[:len(net)], w_minvar)
    bench_maxsh = portfolio_returns_from_weights(tensors.returns[:len(net)], w_maxsh)

    # Output dir per-algo
    out_dir = Path(cfg.paths.outputs_dir) / "backtest" / cfg.train.algo / args.split
    out_dir.mkdir(parents=True, exist_ok=True)
    dates = pd.DatetimeIndex(env.tensors.dates[:len(weights)])

    pd.DataFrame(weights, index=dates, columns=raw["tickers"]).to_csv(out_dir / "daily_weights.csv")
    pd.DataFrame(targets, index=dates, columns=raw["tickers"]).to_csv(out_dir / "fund_target_weights.csv")
    pd.DataFrame(fund_within, index=dates, columns=raw["tickers"]).to_csv(out_dir / "fund_within_sector.csv")
    pd.DataFrame(tilts, index=dates, columns=raw["tickers"]).to_csv(out_dir / "tech_multipliers.csv")
    pd.DataFrame(sector_alloc, index=dates, columns=SECTOR_COLUMNS).to_csv(out_dir / "sector_weights.csv")

    pd.DataFrame({
        "date": dates,
        "gross_return": gross, "net_return": net, "baseline_return": baseline,
        "active_simple_return": active_simple, "active_log_return": active_log,
    ...
    metrics = {
        "split": args.split, "algo": cfg.train.algo,
        "run_name": args.run_name, "fold_name": args.fold_name,
        "n_steps": int(len(net)),
    ...
    for k in ["cumulative_return", "annualized_return", "sharpe", "sortino",
    ...
    for name, comp in metrics["strategy_vs_benchmarks"].items():
        logger.info(f"  {name}: alpha={comp['alpha_total']:+.4f} "
                    f"IR={comp['information_ratio']:+.3f} win={comp['win_rate_vs_benchmark']:.3f}")
    logger.info(f"\nZapisano do: {out_dir}")
